...
// Name validation: only letters and single spaces, 2-50 chars
const NAME_REGEX = /^[A-Za-z]+(?:\s[A-Za-z]+)*$/;
const NAME_MIN_LENGTH = 2;
const NAME_MAX_LENGTH = 50;
const NAME_REQUIREMENTS_MESSAGE = "Name must be 2-50 characters and contain only alphabets.";

/**
 * Normalize a name: trim and collapse multiple spaces to single space
 */
...
/**
 * Validate a name: only letters and single spaces, 2-50 chars
 */
const isValidName = (name) => {
    if (!name || name.length < NAME_MIN_LENGTH || name.length > NAME_MAX_LENGTH) return false;
...
export const Register = async (req, res, next) => {
    try {
        ...
        if (!isValidName(cleanedName)) {
            return next(handleError(400, NAME_REQUIREMENTS_MESSAGE));
        }
...
export const resetPassword = async (req, res, next) => {
    try {
        ...
        // First, find the user and check if password is the same BEFORE consuming the OTP
        const user = await User.findOne({ email: normalizedEmail });
        if (!user) {
            return next(handleError(404, "Account not found."));
        }

        // Check if new password is same as current password BEFORE verifying OTP
        const isSamePassword = await bcryptjs.compare(newPassword, user.password || "");
        if (isSamePassword) {
            return next(handleError(400, "New password cannot be the same as your current password."));
        }

        // Now verify and consume the OTP (only after password validation passes)
