...
const ProfileView = () => {
  ...
  const [followersCount, setFollowersCount] = useState(followStats?.followersCount || 0);
  ...
  const profileBio = profile?.bio?.trim() || "This blogger hasn't shared a bio yet.";
  const topicPreview = categoryLabels.slice(0, 3);
  const blogLabel = blogs.length === 1 ? "blog" : "blogs";
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full">
          <BackButton />
          <div className="mt-6 rounded-4xl border border-rose-200 bg-white p-8 shadow-[0_25px_70px_-55px_rgba(15,23,42,0.7)] text-center">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-rose-100 text-rose-600">
              <Users className="h-10 w-10" />
            </div>
            <h2 className="text-2xl font-bold text-slate-900 mb-3">Unable to Load Profile</h2>
            <p className="text-slate-600 mb-6">
              We couldn't load this profile right now. This could be due to a network issue or the profile may not exist.
            </p>
      ...
      <div className="max-w-6xl px-4 py-10 mx-auto space-y-10 sm:px-8 lg:px-12">
        ...
        <section className="rounded-4xl border border-slate-100 bg-white/95 p-6 shadow-[0_25px_70px_-55px_rgba(15,23,42,0.7)] space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.35em] text-slate-500">Published blogs</p>
            ...
            <span className="px-4 py-2 text-sm font-medium border rounded-full border-slate-200 bg-slate-50 text-slate-600">
              {formatNumber(blogs.length)} {blogLabel}
            </span>
          </div>
          ...
          {blogs.length === 0 ? (
            <div className="p-12 text-center border border-dashed rounded-3xl border-slate-200 bg-slate-50/80">
              ...
              <p className="text-lg font-semibold text-slate-800">No blogs published yet</p>
