...
const FeaturedCard = () => {
  ...
  const getExcerpt = (blog) => {
    ...
    if (!rawContent) return "Dive into the blog and explore more.";

    try {
      const plain = decode(rawContent.replace(/<[^>]+>/g, " "))
        .replace(/\s+/g, " ")
        .trim();
      if (!plain) return "Dive into the blog and explore more.";
      return plain.length > 150 ? `${plain.slice(0, 147)}...` : plain;
    } catch {
      return "Dive into the blog and explore more.";
    }
  };
  ...
  return (
    <div className="col-span-full bg-[#6C5CE7] rounded-4xl sm:rounded-[40px] mb-10 relative overflow-hidden text-white shadow-xl shadow-indigo-200 min-h-[420px] md:min-h-[460px]">
      ...
      <div className="absolute bottom-0 w-48 h-48 translate-y-1/2 bg-purple-300 rounded-full left-20 opacity-20 blur-3xl" />
                ...
                <div className="flex flex-col gap-5">
                  {currentCategory && (
                    <motion.span
                      ...
                      className="inline-flex w-fit items-center justify-center bg-white/20 backdrop-blur-md text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-[0.3em] border border-white/10"
                    >
                      {currentCategory}
                    </motion.span>
                  )}
