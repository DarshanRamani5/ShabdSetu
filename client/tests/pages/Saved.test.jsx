...
describe('Saved Page', () => {
  ...
  it('displays saved blogs count', () => {
    ...
    expect(screen.getByText(/ready to revisit/i)).toBeInTheDocument();
  });
  ...
  it('filters blogs by category', async () => {
    ...
    await waitFor(() => {
      const shownElements = screen.getAllByText(/1 shown/i);
      expect(shownElements[0]).toBeInTheDocument();
    });
  });

  it('shows all categories button and filters correctly', async () => {
    renderComponent();
    ...
    await waitFor(() => {
      const shownElements = screen.getAllByText(/3 shown/i);
      expect(shownElements[0]).toBeInTheDocument();
    });
  });
